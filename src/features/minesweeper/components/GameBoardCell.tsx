...
export function GameBoardCell({ id }: Props) {
  ...
  switch (state) {
    case CellState.REVEALED:
      return <div className={gameboardCell({ state: CellState.REVEALED, count: neigboringMines })}>{neigboringMines}</div>;
    case CellState.MINE_REVEALED:
      return (
        <div className={gameboardCell({ state: CellState.MINE_REVEALED })}>
          <BombIcon />
        </div>
      );
    case CellState.MINE_EXPLODED:
      return (
        <div className={gameboardCell({ state: CellState.MINE_EXPLODED })}>
          <BombIcon />
        </div>
      );
    case CellState.MINE_CORRECT_FLAGGED:
      return (
        <div className={gameboardCell({ state: CellState.MINE_CORRECT_FLAGGED })}>
          <FlagIcon />
        </div>
      );
    case CellState.MINE_INCORRECT_FLAGGED:
      return (
        <div className={gameboardCell({ state: CellState.MINE_INCORRECT_FLAGGED })}>
          <FlagIcon />
        </div>
      );
    case CellState.FLAGGED:
      return (
        <button className={gameboardCell({ state: CellState.FLAGGED })} onClick={unflag} onContextMenu={unflag}>
          <FlagIcon />
        </button>
      );
    default:
...
function BombIcon() {
  return (
    <div className={iconContainer}>
      <svg xmlns="http://www.w3.org/2000/svg" aria-label="bomb" focusable="false" role="img" viewBox="0 0 512 512">
        <path
          fill="currentColor"
...
function FlagIcon() {
  return (
    <div className={iconContainer}>
      <svg xmlns="http://www.w3.org/2000/svg" aria-label="flag" focusable="false" role="img" viewBox="0 0 512 512">
        <path
          fill="currentColor"
